...
async function run() {
  console.log("→ Parsing local sheet data...");
  const subjectsMap = parseRawData(RAW_DATA);
  console.log(`Parsed ${subjectsMap.size} distinct refined subjects.`);

  console.log("→ Scanning current subjects from fep-subjects...");
  const scanRes = await ddb.send(new ScanCommand({ TableName: TABLES.SUBJECTS }));
  console.log(`Found ${scanRes.Items?.length ?? 0} existing subjects to delete.`);

  for (const item of scanRes.Items ?? []) {
    await ddb.send(new DeleteCommand({ TableName: TABLES.SUBJECTS, Key: { subjectId: item.subjectId } }));
  }
  console.log("✓ Cleared fep-subjects table.");

  console.log("→ Populating subjects from sheet...");
  let count = 0;
  ...
  console.log(`✓ Successfully synced ${count} subjects into DynamoDB!`);

  console.log("→ Scanning all videos to update subject mappings...");
  const videosRes = await ddb.send(new ScanCommand({ TableName: "fep-videos" }));
  const videos = videosRes.Items ?? [];
  for (const v of videos) {
    let vert = v.subjectId || "foundation";
    if (vert.includes("-")) {
      vert = vert.split("-").pop() || "foundation";
    }
    const refined = refineSubjectAndVertical(v.subject || "General Studies", vert);
    console.log(`  Mapping video "${v.title}" (${v.subject} -> ${refined.name} [${refined.subjectId}])`);
    
    // Delete the old video first to prevent partition key / range key index mismatches if indexing depends on subjectId
    // Wait, the index subjectId-uploadedAt-index GSI will automatically update when we update the attributes.
    // However, subjectId is NOT part of the primary key (primary key is HASH facultyId, RANGE videoId).
    // So simple PutCommand with updated subjectId and subject will overwrite/update it correctly.
    await ddb.send(new PutCommand({
      TableName: "fep-videos",
      Item: {
        ...v,
        subject: refined.name,
        subjectId: refined.subjectId
      }
    }));
  }
  console.log(`✓ Updated mappings for ${videos.length} videos.`);
}
