...
async function run() {
  console.log(`Starting extra users deletion. DRY_RUN = ${DRY_RUN}`);
  ...
  for (const u of dbUsers) {
    if (u.cohort === "March EduSkill") continue;
    if (u.role === "eduskill_admin" || u.role === "eduskill_manager") continue;

    const norm = cleanEmail(String(u.email));
    if (!targetEmails.has(norm)) {
      const associatedVideos = dbVideos.filter(v => v.facultyId === u.userId);
      extraUsers.push({ user: u, videos: associatedVideos });
    }
  }

  console.log(`Found ${extraUsers.length} extra/invalidated users.`);

  for (const item of extraUsers) {
    const { user, videos } = item;
    console.log(`[DELETE] User "${user.name}" (${user.email}) [Cohort: ${user.cohort}]`);
    
    // Delete associated videos
    for (const v of videos) {
      console.log(`  - Delete Video "${v.title}" (${v.videoId})`);
      if (!DRY_RUN) {
